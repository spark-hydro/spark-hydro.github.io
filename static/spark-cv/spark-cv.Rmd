...
```{r}
PDF <- TRUE # Set to TRUE when rendering to PDF via pagedown::chrome_print('spark-cv.Rmd', 'spark-cv.pdf')
if(PDF) {
  header <- '
...
<div class = "footer",>
  <a href="https://github.com/spark-hydro" class="icon-block">
      <i class="fab fafooter fa-github"> &ensp; | </i>
  </a>
  
  <a href="mailto:seonggyu.park@ttu.edu" class="icon-block">
      <i class="fas fafooter fa-envelope"> &ensp;| </i>
  </a>
  
  <a href="https://spark-hydro.github.io/" class="icon-block">
...
<div class="item2">
<div id = "block_container">
<div class = "first2", id="bloc1">Assistant</div>
<div class = "last2", id="bloc2">Professor</div>
<div class = "last2-1", id="bloc2">of</div>
<div class = "last2", id="bloc2">Hydrologic Modeling</div>
</div>
</div>

<!-- Online -->
<!-- <div class="item3"> -->
<!-- <a href="https://github.com/spark-hydro/spark-hydro.github.io/raw/main/spark-cv/spark-cv.pdf" class="icon-block"> -->
...
<!-- PDF -->
<div class="item3">
<a href="https://spark-hydro.github.io/spark-cv/" class="icon-block">
...
<div class = "hangingindent prpa nohead">
```{r} 
kable(pubs_tbl[24:nrow(pubs_tbl), ])
```
</div>
...
```{r}
kable(conf_tbl[31:nrow(conf_tbl), ])
```
</div>
...
## University service {.history .nohead}
```{r}
cv_entries %>% 
  filter(type == 'service') %>% 
  mutate(
...
## Dissertation and Thesis Advising {.history .nohead}
...
cources <- cources %>%
  mutate(
    ...
    what = paste0("<span style= 'font-weight: 500'>",
                       ...
                       "</span>"),
    pps = ifelse(!is.na(participants),
                paste0("<i class='fas location fa-users'></i>", "Enrollment: ", participants),
                ""),
    what = paste0(what, ", ", where, ", ", loc, ", ",pps),
         what = gsub("NA", "", what),
         what = gsub(", $", "", what)) %>% 
  mutate(year = str_c(year_begin, 
                      # " --- ", 
